...
class GameModel {//removed Codable ASB 06/29/19 //changed struct to class, added NSCoding 7/27/19
    ...
    var pits: CircularLinkedList_PN // ASB 06/29/19
    ...
    let player1Perspective = [//ASB 07/06/19
    ...
    let player2Perspective = [//player 2's perspective, P1 on top
    ...
    convenience init(from gkMatchData: Data) {
        ...
        if !nsData.isEmpty {
            do {
                ...
                activePlayer = playerTurn == 1 ? mancalaPlayer1 : mancalaPlayer2
                turnNumber = gameData.turnNumber
//                var changePerspective = gameData.playerPerspective
//                changePerspective = changePerspective == 1 ? 2 : 1
//                playerPerspective = changePerspective
//                positions = playerPerspective == 1 ? player1Perspective : player2Perspective
                playerTurnText = gameData.playerTurnText
                winner = gameData.winner
    ...
    func registerConcreteSubclasses() {
        let nodePitnode = NodeTypePitNode(PitNode())
        let nodePitNodeClass = NodeTypePitNode.self
        let nodePitClassName = nodePitnode.theClassName
        ...
        let circLinkListPitNodeClass = type(of: pits)
        let circLinkListPitNodeClassName = pits.theClassName
    ...
    //mutating
    func buildGameboard(_ pits: CircularLinkedList_PN, pitsPerPlayer: Int ) {
    ...
    func buildGameboardTEST(_ pits: CircularLinkedList_PN, pitsPerPlayer: Int ) {
    ...
    func buildGameboardTEST_END_GAME(_ pits: CircularLinkedList_PN, pitsPerPlayer: Int ) {
...
//    func transferCoordinatesToPits() {
//        let pitsIterator = pits.circIter
//        ++pitsIterator //go to Ply 1, pit #1
//        var i = 0
//        repeat {
//            let tempPit = *pitsIterator
//            tempPit?.coord = positions[i]
//            ++pitsIterator
//            i += 1
//        } while i < pits.length
//        
//    }
    ...
    func printBoard(_ gameboard: CircularLinkedList_PN) {
