...
public class MancalaPlayer {
    ...
    public var copiedBoard: CircularLinkedList<PitNode>?
    ...
    public func copyBoard(from originalBoard: CircularLinkedList<PitNode>){
        
        let myIter = originalBoard.circIter
        ++myIter
        copiedBoard = CircularLinkedList<PitNode>()
        
        for _ in 1...originalBoard.length {
            if let pit = *myIter {
                let newPit = PitNode()
                newPit.player = pit.player
                newPit.beads = pit.beads
                newPit.name = pit.name
