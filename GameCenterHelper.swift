...
//            var numWonGames = UserDefaults.numberOfWonGames

            //add to dictionary of [matchIDs : matchOutcome]
            //save matchDictionary to Documents directory
            //if let outcomes = matchDictionary.values {
                ...
                for match in allActiveMatches {
                    if match.status == .ended {
                        for participant in match.participants {
                            if let participantID = participant.player?.playerID {
                                if GKLocalPlayer.local.playerID == participantID, participant.matchOutcome == .won {
                                    //{
                                    //            var numWonGames = UserDefaults.numberOfWonGames
                                                //check matchDictionary.keys if matchID does not exist
